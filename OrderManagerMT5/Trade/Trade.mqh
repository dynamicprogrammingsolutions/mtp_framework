...
public:
   ...
   bool              PositionOpen(const string __symbol,ENUM_ORDER_TYPE order_type,double volume,
                                  double price,double sl,double tp,const string comment="");
   bool              PositionModify(const string __symbol,double sl,double tp);
   bool              PositionClose(const string __symbol,ulong deviation=ULONG_MAX);
   //--- methods for working with pending orders
   bool              OrderOpen(const string __symbol,ENUM_ORDER_TYPE order_type,double volume,
   ...
   //--- additions methods
   bool              Buy(double volume,const string __symbol=NULL,double price=0.0,double sl=0.0,double tp=0.0,const string comment="");
   bool              Sell(double volume,const string __symbol=NULL,double price=0.0,double sl=0.0,double tp=0.0,const string comment="");
   bool              BuyLimit(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                              ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="");
   bool              BuyStop(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                             ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="");
   bool              SellLimit(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                               ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="");
   bool              SellStop(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                              ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="");
   //--- method check
   ...
   string            FormatOrderPrice(string& str,const double price_order,const double price_trigger,const uint __digits) const;
   //--- trade request
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::PositionOpen(const string __symbol,ENUM_ORDER_TYPE order_type,double volume,
                          double price,double sl,double tp,const string comment)
  {
//--- check stopped
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::PositionModify(const string __symbol,double sl,double tp)
  {
//--- check stopped
   if(IsStopped(__FUNCTION__)) return(false);
//--- clean
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::PositionClose(const string __symbol,ulong deviation)
  {
...
//--- clean
   ...
   do
     {
      //--- checking
      if(PositionSelect(__symbol))
        {
         if((ENUM_POSITION_TYPE)PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY)
           {
            //--- prepare request for close BUY position
            m_request.type =ORDER_TYPE_SELL;
            m_request.price=SymbolInfoDouble(__symbol,SYMBOL_BID);
           }
         else
           {
            //--- prepare request for close SELL position
            m_request.type =ORDER_TYPE_BUY;
            m_request.price=SymbolInfoDouble(__symbol,SYMBOL_ASK);
           }
        }
      else
        {
      ...
      //--- check volume
      double max_volume=SymbolInfoDouble(__symbol,SYMBOL_VOLUME_MAX);
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::OrderOpen(const string __symbol,ENUM_ORDER_TYPE order_type,double volume,double limit_price,
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::Buy(double volume,const string __symbol=NULL,double price=0.0,double sl=0.0,double tp=0.0,const string comment="")
  {
   CSymbolInfo sym;
//--- check volume
   if(volume<=0.0)
...
//--- check symbol
   sym.Name((__symbol==NULL)?Symbol():__symbol);
//--- check price
   if(price==0.0)
     {
      sym.RefreshRates();
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::Sell(double volume,const string __symbol=NULL,double price=0.0,double sl=0.0,double tp=0.0,const string comment="")
  {
   CSymbolInfo sym;
//--- check volume
   if(volume<=0.0)
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::BuyLimit(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                      ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="")
  {
   string sym;
//--- check volume
...
//--- check symbol
   sym=(__symbol==NULL)?Symbol():__symbol;
//--- send "BUY_LIMIT" order
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::BuyStop(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                     ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="")
  {
   string sym;
//--- check volume
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::SellLimit(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                       ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="")
  {
   string sym;
//--- check volume
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::SellStop(double volume,double price,const string __symbol=NULL,double sl=0.0,double tp=0.0,
                      ENUM_ORDER_TYPE_TIME type_time=ORDER_TIME_GTC,datetime expiration=0,const string comment="")
  {
   string sym;
//--- check volume
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
string CTrade::FormatOrderPrice(string& str,const double price_order,const double price_trigger,const uint __digits) const
  {
...
//--- set up
   ...
   if(request.symbol!=NULL)
     {
      if(__symbol.Name(request.symbol))
         __digits=__symbol.Digits();
     }
//--- see what is wanted
   switch(request.action)
     {
      ...
      case TRADE_ACTION_DEAL:
         switch(__symbol.TradeExecution())
           {
            //--- request execution
            case SYMBOL_TRADE_EXECUTION_REQUEST:
               str=StringFormat("request %s %s %s at %s",
                                ...
                                request.symbol,
                                DoubleToString(request.price,__digits));
            //--- Is there SL or TP?
            if(request.sl) { tmp=StringFormat(" sl: %s",DoubleToString(request.sl,__digits)); str+=tmp; }
            if(request.tp) { tmp=StringFormat(" tp: %s",DoubleToString(request.tp,__digits)); str+=tmp; }
            break;
            //--- instant execution
      ...
      case TRADE_ACTION_PENDING:
         str=StringFormat("%s %s %s at %s",
                          ...
                          DoubleToString(request.volume,2),
                          request.symbol,
                          FormatOrderPrice(price,request.price,request.stoplimit,__digits));
      //--- Is there SL or TP?
      if(request.sl) { tmp=StringFormat(" sl: %s",DoubleToString(request.sl,__digits)); str+=tmp; }
      if(request.tp) { tmp=StringFormat(" tp: %s",DoubleToString(request.tp,__digits)); str+=tmp; }
      break;
      ...
      case TRADE_ACTION_SLTP:
         str=StringFormat("modify %s (sl: %s, tp: %s)",
                          request.symbol,
                          DoubleToString(request.sl,__digits),
                          DoubleToString(request.tp,__digits));
      break;
      ...
      case TRADE_ACTION_MODIFY:
         str=StringFormat("modify #%I64u at %s (sl: %s tp: %s)",
                          request.order,
                          FormatOrderPrice(price_new,request.price,request.stoplimit,__digits),
                          DoubleToString(request.sl,__digits),
                          DoubleToString(request.tp,__digits));
      break;
   ...
   switch(result.retcode)
     {
      case TRADE_RETCODE_REQUOTE:
         str=StringFormat("requote (%s/%s)",
                          DoubleToString(result.bid,__digits),
                          DoubleToString(result.ask,__digits));
      break;

      case TRADE_RETCODE_DONE:
         if(request.action==TRADE_ACTION_DEAL &&
           (__symbol.TradeExecution()==SYMBOL_TRADE_EXECUTION_REQUEST ||
            __symbol.TradeExecution()==SYMBOL_TRADE_EXECUTION_INSTANT ||
            __symbol.TradeExecution()==SYMBOL_TRADE_EXECUTION_MARKET))
            str=StringFormat("done at %s",DoubleToString(result.price,__digits));
         else
            str="done";
      break;
         ...
         if(request.action==TRADE_ACTION_DEAL &&
           (__symbol.TradeExecution()==SYMBOL_TRADE_EXECUTION_REQUEST ||
            ...
            str=StringFormat("done partially %s at %s",
                             ...
                             DoubleToString(result.price,__digits));
         else
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CTrade::OrderCheck(MqlTradeRequest& __m_request,MqlTradeCheckResult& __m_check_result)
  {
   return(::OrderCheck(__m_request,__m_check_result));
  }
//+------------------------------------------------------------------+

...
public:
   ...
   bool              Select(const string __symbol);
   bool              SelectByIndex(uint index);
   //---
   void              StoreState();
   bool              CheckState();
  };
...
//--- form the position description
   str=StringFormat("%s %s %s %s",
                    ...
                    Symbol(),
                    DoubleToString(PriceOpen(),__digits+3));
//--- add stops if there are any
   double sl=StopLoss();
   double tp=TakeProfit();
   if(sl) { tmp=StringFormat(" sl: %s",DoubleToString(sl,__digits)); str+=tmp; }
   if(tp) { tmp=StringFormat(" tp: %s",DoubleToString(tp,__digits)); str+=tmp; }
//--- return the result
   return(str);
  }
//+------------------------------------------------------------------+
//| Access functions PositionSelect(...).                            |
//| INPUT:  __symbol -__symbol name for select position.                 |
//| OUTPUT: true-if successful, false otherwise.                     |
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CPositionInfo::Select(const string __symbol)
  {
   return(PositionSelect(__symbol));
  }
//+------------------------------------------------------------------+
...
//| REMARK: no.                                                      |
//+------------------------------------------------------------------+
bool CPositionInfo::SelectByIndex(uint index)
  {

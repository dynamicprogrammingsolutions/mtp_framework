...
bool ordertype_pending(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY_LIMIT:
      case ORDER_TYPE_SELL_LIMIT:
      case ORDER_TYPE_BUY_STOP:
...
bool ordertype_stop(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY_STOP:
      case ORDER_TYPE_SELL_STOP:
         return(true);
...
bool ordertype_limit(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY_LIMIT:
      case ORDER_TYPE_SELL_LIMIT:
         return(true);
...
bool ordertype_market(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY:
      case ORDER_TYPE_SELL:
         return(true);
...
bool ordertype_pendinglong(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY_LIMIT:
      case ORDER_TYPE_BUY_STOP:
         return(true);
...
bool ordertype_long(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_BUY:
      case ORDER_TYPE_BUY_LIMIT:
      case ORDER_TYPE_BUY_STOP:
...
bool ordertype_short(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_SELL:
      case ORDER_TYPE_SELL_LIMIT:
      case ORDER_TYPE_SELL_STOP:
...
bool ordertype_pendingshort(ENUM_ORDER_TYPE in_ordertype) {
   switch (in_ordertype) {
      case ORDER_TYPE_SELL_LIMIT:
      case ORDER_TYPE_SELL_STOP:
         return(true);
...
bool ordertype_select(ENUM_ORDERSELECT type, ENUM_ORDER_TYPE ordtype)
{
...
/*
double slprice(int cmd, int in_stoploss, double price=0, bool formodify = false)
{
   if (!formodify && in_stoploss <= 0)
      in_stoploss = IEMPTY_VALUE;

   if (in_stoploss == IEMPTY_VALUE)
      return(0);

   if ((cmd == OP_BUY) || (cmd == OP_BUYLIMIT) || (cmd == OP_BUYSTOP))
   {
      if (price == 0) price = ask;
      return(price - in_stoploss*ticksize);
   }
   ...
   Alert("slprice: ",errmsg_invalidtype);
   return(0);
}

double tpprice(int cmd, int in_takeprofit, double price=0, bool formodify = false)
{
   if (!formodify && in_takeprofit <= 0)
      in_takeprofit = IEMPTY_VALUE;

   if (in_takeprofit == EMPTY_VALUE)
      return(0);

   if ((cmd == OP_BUY) || (cmd == OP_BUYLIMIT) || (cmd == OP_BUYSTOP))
   {
      if (price == 0) price = ask;
      return(price + in_takeprofit*ticksize);
   }
   if ((cmd == OP_SELL) || (cmd == OP_SELLLIMIT) || (cmd == OP_SELLSTOP))
   {
      if (price == 0) price = bid;
   ...
   Alert("tpprice: ",errmsg_invalidtype);
   return(0);
}
*/

double getstoplossprice(string in_symbol, ENUM_ORDER_TYPE in_ordertype, int in_stoploss, double price=0, bool formodify = false)
{
   if (price == 0) {
...
double gettakeprofitprice(string in_symbol, ENUM_ORDER_TYPE in_ordertype, int in_stoploss, double price=0, bool formodify = false)
{
   if (price == 0) {
      price = getentryprice(in_symbol,in_ordertype,0,0);      
   }
...
double getentryprice(string in_symbol, ENUM_ORDER_TYPE in_ordertype, int entrydistance, double price = 0)
{
...
int getstoplossticks(string in_symbol, ENUM_ORDER_TYPE in_ordertype, double in_stoploss, double price=0)
{
...
int gettakeprofitticks(string in_symbol, ENUM_ORDER_TYPE in_ordertype, double in_takeprofit, double price=0)
{
   if (in_takeprofit == 0)
      return(0);
...
int getprofitticks(string in_symbol, ENUM_ORDER_TYPE in_ordertype, double closeprice, double entryprice)
{
   if (ordertype_long(in_ordertype)) {
...
int getentrypriceticks(string in_symbol,ENUM_ORDER_TYPE in_ordertype, double entryprice, double price=0)
{
   loadsymbol(in_symbol,__FUNCTION__);
...
double getcurrententryprice(string in_symbol, ENUM_ORDER_TYPE in_ordertype)
{
   loadsymbol(in_symbol,__FUNCTION__);
...
double getcurrentcloseprice(string in_symbol, ENUM_ORDER_TYPE in_ordertype)
{
   loadsymbol(in_symbol,__FUNCTION__);
   double currentprice = 0;

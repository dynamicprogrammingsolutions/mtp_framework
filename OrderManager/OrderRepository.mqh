...
public:
   ...
   TraitGetType(classOrderRepository)
   ...
   virtual bool SelectByTicketHistory(uint ticket) { int idx; return(((idx = GetIdxByTicketHistory(ticket)) >= 0)?SelectByIdxOrder(idx):false); }
   ...
   virtual void  OnTradeTransaction(
      ...
      for (int i = 0; i < orders.Total(); i++) {
         ...
         _order = orders.At(i);
         _order.OnTradeTransaction(trans,request,result);
      }
   }
   #endif

...
class CTrailingPending : public CTrailing
{
public:
   TraitAppAccess
   ...
   virtual CStopsCalcInterface* CalcSL(COrderInterface* in_order)
   {
      return NULL;
   }
   
   virtual CStopsCalcInterface* CalcTP(COrderInterface* in_order)
   {
      return NULL;
   }
   
   virtual CStopsCalcInterface* CalcEntry(COrderInterface* in_order)
   {
      return NULL;
   }
   ...
   {
      if (!isset(in_order)) return false;
      shared_ptr<CStopsCalcInterface> newsl = CalcSL(in_order);
      shared_ptr<CStopsCalcInterface> newtp = CalcTP(in_order);
      shared_ptr<CStopsCalcInterface> newentry = CalcEntry(in_order);
      if (newentry.isset()) {
         in_order.SetPrice(newentry.get(),true);
      }
      if (newsl.isset()) {
         in_order.SetStopLoss(newsl.get(),true);
      }
      if (newtp.isset()) {
         in_order.SetTakeProfit(newtp.get(),true);
      }
      bool ret = in_order.Modify();
      return ret;
      return false;
